...
double innerproduct(vector<complex<double>>& a, vector<complex<double>>& b, int N) {
    vector<complex<double>> res(1), innres(N);
    MKL_Complex16 one = { 1, 0 };
    MKL_Complex16 zero = { 0, 0 };
    cblas_zgemm(CblasRowMajor, CblasConjTrans, CblasNoTrans, 1, N, N, &one, a.data(), 1, b.data(), N, &zero, innres.data(), N);
    cblas_zgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, N, 1, 1, &one, innres.data(), N, a.data(), N, &zero, res.data(), N);
    return norm(res[0]);
}
